...
| `discovery/leaderboard.py` | **STREAM A.** Polls the service board on `leaderboard.interval_sec`, pages until it has enough names, and FOLLOWS them directly — the service already ranked and vetted them. Configured solely by the `leaderboard` section; `filters` never applies. The stream runs whenever `chain.solana_tracker_api_key` is set — the key is the only on/off switch. Score = board position (not the win-rate field, which a never-sell-losers wallet reports as ~100%). |
